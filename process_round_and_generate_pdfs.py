...
def get_connection():
    ...
    if missing:
        raise SystemExit("Missing required environment value(s): " + ", ".join(missing) + ". Add them to app/.env or the environment before running this script.")
...
def fetch_letter_and_make_pdf(cur, letter_id: str):
    ...
    # Attach PDF to client record (doc_type must match your enum; attach_letter_pdf uses 'dispute_letter')
    cur.execute(
...
def main(client_id: str, round_number: int, include_personal_info: bool, client_email: str):
    ...
    with conn.cursor() as cur:
        # This DB function generates 3 letters + logs the update email, and returns JSON with IDs
        cur.execute("""
            SELECT process_dispute_round_json(%s, %s, %s, %s, %s, %s)::text
        """, (
            client_id,
            round_number,
        ...
        for item in result["letters"]:
            ...
            pdf_path = fetch_letter_and_make_pdf(cur, letter_id)
            print(f"{bureau}: {letter_id}")
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        raise SystemExit(
            "Usage: python process_round_and_generate_pdfs.py <client_id> <round_number> <include_personal_info:true|false> <client_email>"
        )
