...
def load_client_workspace_context(cur, client_id: str, reveal_cred_id: Optional[str] = None):
    ...
    return dashboard, profile, score_groups, credentials, notes, appointments, referral_partners, negative_items, personal_info_errors, credit_products, outbound_referrals


def render_client_workspace(request: Request, client_id: str, message: str = "", error: str = "", reveal_cred_id: Optional[str] = None):
    conn = get_conn()
    try:
        with conn.cursor() as cur:
            dashboard, profile, score_groups, credentials, notes, appointments, referral_partners, negative_items, personal_info_errors, credit_products, outbound_referrals = load_client_workspace_context(cur, client_id, reveal_cred_id)
        if not profile:
...
def ui_add_note(
    ...
    try:
        ...
        conn.close()
        return render_client_workspace(request, client_id, message="Note added.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))


@app.post("/ui/delete-note", response_class=HTMLResponse)
def ui_delete_note(request: Request, client_id: str = Form(...), note_id: str = Form(...)):
    try:
        conn = get_conn()
        conn.autocommit = True
        with conn.cursor() as cur:
        ...
        conn.close()
        return render_client_workspace(request, client_id, message="Note deleted.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))
...
def ui_add_appointment(
    ...
    try:
        ...
        conn.close()
        return render_client_workspace(request, client_id, message="Appointment saved. Email/SMS automation hooks are ready for the next step.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))


@app.post("/ui/delete-appointment", response_class=HTMLResponse)
def ui_delete_appointment(request: Request, client_id: str = Form(...), appointment_id: str = Form(...)):
    try:
        conn = get_conn()
        conn.autocommit = True
        with conn.cursor() as cur:
        ...
        conn.close()
        return render_client_workspace(request, client_id, message="Appointment deleted.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))
...
def ui_reveal_credential(request: Request, client_id: str = Form(...), credential_id: str = Form(...)):
    try:
        return render_client_workspace(request, client_id, message="Credential revealed on this screen only.", reveal_cred_id=credential_id)
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))
...
def ui_save_credential(
    ...
    try:
        ...
        conn.close()
        return render_client_workspace(request, client_id, message="Credential saved.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))
...
def ui_delete_credential(request: Request, client_id: str = Form(...), credential_id: str = Form(...)):
    try:
        ...
        conn.close()
        return render_client_workspace(request, client_id, message="Credential deleted.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))
...
def ui_add_score(
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            """, (client_id, report_date, bureau, score, model, source))
        conn.close()
        return render_client_workspace(request, client_id, message="Score saved.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))
...
def ui_add_myfico(
    ...
    try:
        ...
        conn.close()
        return render_client_workspace(request, client_id, message="MyFICO scores saved.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))
...
def ui_delete_score(request: Request, client_id: str = Form(...), snapshot_id: str = Form(...)):
    try:
        ...
        conn.close()
        return render_client_workspace(request, client_id, message="Score entry deleted.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))
...
def ui_add_negative_item(
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            """, (client_id, bureau, creditor_name, account_number, account_type, dispute_reason, current_status, notes))
        conn.close()
        return render_client_workspace(request, client_id, message="Negative/dispute item saved.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))


@app.post("/ui/delete-negative-item", response_class=HTMLResponse)
def ui_delete_negative_item(request: Request, client_id: str = Form(...), item_id: str = Form(...)):
    try:
        conn = get_conn(); conn.autocommit = True
        with conn.cursor() as cur:
            cur.execute("DELETE FROM client_negative_items WHERE id = %s AND client_id = %s", (item_id, client_id))
        conn.close()
        return render_client_workspace(request, client_id, message="Negative/dispute item deleted.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))
...
def ui_add_personal_info_error(
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            """, (client_id, bureau, error_type, value_text, dispute_action, current_status, notes))
        conn.close()
        return render_client_workspace(request, client_id, message="Personal information error saved.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))


@app.post("/ui/delete-personal-info-error", response_class=HTMLResponse)
def ui_delete_personal_info_error(request: Request, client_id: str = Form(...), item_id: str = Form(...)):
    try:
        conn = get_conn(); conn.autocommit = True
        with conn.cursor() as cur:
            cur.execute("DELETE FROM client_personal_info_errors WHERE id = %s AND client_id = %s", (item_id, client_id))
        conn.close()
        return render_client_workspace(request, client_id, message="Personal information error deleted.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))
...
def ui_add_credit_product(
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            """, (client_id, lender_name, credit_type, credit_limit, due_date, cutoff_date, secured_deposit_amount, origination_date, notes))
        conn.close()
        return render_client_workspace(request, client_id, message="Credit product saved.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))


@app.post("/ui/delete-credit-product", response_class=HTMLResponse)
def ui_delete_credit_product(request: Request, client_id: str = Form(...), item_id: str = Form(...)):
    try:
        conn = get_conn(); conn.autocommit = True
        with conn.cursor() as cur:
            cur.execute("DELETE FROM client_credit_products WHERE id = %s AND client_id = %s", (item_id, client_id))
        conn.close()
        return render_client_workspace(request, client_id, message="Credit product deleted.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))
...
def ui_save_referral_partner(
    ...
    try:
        ...
        conn.close()
        return render_client_workspace(request, client_id, message="Referral partner saved.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))
...
def ui_add_outbound_referral(
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            """, (client_id, referral_partner_id, referral_date, referral_reason, status, notes))
        conn.close()
        return render_client_workspace(request, client_id, message="Outbound referral saved.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))


@app.post("/ui/delete-outbound-referral", response_class=HTMLResponse)
def ui_delete_outbound_referral(request: Request, client_id: str = Form(...), item_id: str = Form(...)):
    try:
        conn = get_conn(); conn.autocommit = True
        with conn.cursor() as cur:
            cur.execute("DELETE FROM client_outbound_referrals WHERE id = %s AND client_id = %s", (item_id, client_id))
        conn.close()
        return render_client_workspace(request, client_id, message="Outbound referral deleted.")
    except Exception as e:
        return render_client_workspace(request, client_id, error=str(e))
